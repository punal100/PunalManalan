...
function cmdValidateConsistency(cwd, raw) {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      // Summary without matching plan is suspicious only for plan-scoped summaries.
      // Phase-level summaries such as `09.8-SUMMARY.md` or `09.6-EXECUTION-SUMMARY.md`
      // are legitimate artifacts and should not be treated as missing plans.
      for (const summary of summaries) {
        const sid = summary.replace("-SUMMARY.md", "");
        const summaryPath = path.join(phasesDir, dir, summary);
        const summaryContent = fs.readFileSync(summaryPath, "utf-8");
        const summaryFm = extractFrontmatter(summaryContent);
        const isPlanSummary =
          summaryFm.plan !== undefined || /-\d{2}$/.test(sid);

        if (isPlanSummary && !planIds.has(sid)) {
          warnings.push(
    ...
    try {
      ...
      if (currentPhaseMatch) {
        ...
        if (phaseCheckboxRe.test(roadmapContentFull)) {
          ...
          const statusVal = stateStatus
            ? stateStatus[1].trim().toLowerCase()
            : "";
          ...
          case "resetConfig": {
            const defaults = {
              ...
              milestone_branch_template:
                CONFIG_DEFAULTS.milestone_branch_template,
...
function cmdVerifySchemaDrift(cwd, phaseArg, skipFlag, raw) {
  ...
  const result = checkSchemaDrift(allFiles, executionLog, {
    skipCheck: !!skipFlag,
  });
